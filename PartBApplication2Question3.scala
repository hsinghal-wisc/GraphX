...
object PartBApplication2Question3 {
  def main(args: Array[String]) {
    ...
    println("cartesian begin ")
    cartesian_users.foreach(println)
    ...
    val wordCountNeighbourVertices: VertexRDD[(Int, Double)] = graph.aggregateMessages[(Int, Double)](
    ...
    val avgNoWordsNeighbourVertices: VertexRDD[Double] =
      wordCountNeighbourVertices.mapValues( (id, value) =>
        value match { case (count, totalWords) => totalWords / count } )
    // Display the results
    println("<<<<<<<<<<#############################>>>>>>>>>>")

    println("<<<<< Average no of words in neighbour vertices are  >>>>>")

    avgNoWordsNeighbourVertices.collect.foreach(println(_))
